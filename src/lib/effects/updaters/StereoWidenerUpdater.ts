...
/**
 * Updater para efectos StereoWidener
 * Implementa el Strategy Pattern específico para StereoWidener
 */
export class StereoWidenerUpdater implements IEffectUpdater<Tone.StereoWidener> {
  readonly effectType = 'StereoWidener';

  /**
   * Actualiza los parámetros de un efecto StereoWidener
   * @param effect Nodo StereoWidener a actualizar
   * @param params Parámetros a aplicar
   */
  updateParams(effect: Tone.StereoWidener, params: EffectParams): void {
    ...
    console.log('StereoWidener actualizado:', {
      width: effect.width.value,
  ...
  canUpdate(effect: Tone.ToneAudioNode): boolean {
    return effect instanceof Tone.StereoWidener;
  }
   ...
   */
  getCurrentParams(effect: Tone.StereoWidener): Record<string, unknown> {
    return {
      width: effect.width.value,
   ...
   */
  private safeUpdateParam(effect: Tone.StereoWidener, paramName: string, value: number | string): boolean {
    try {
      switch (paramName) {
        case 'width':
          effect.width.value = value as number;
          break;
        case 'wet':
          if (effect.wet) {
      ...
      console.warn(`⚠️ StereoWidenerUpdater: Error actualizando parámetro ${paramName}:`, error);
      return false;
    }
  }
}
