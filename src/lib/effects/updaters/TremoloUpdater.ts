...
/**
 * Updater para efectos Tremolo
 * Implementa el Strategy Pattern específico para Tremolo
 */
export class TremoloUpdater implements IEffectUpdater<Tone.Tremolo> {
  readonly effectType = 'Tremolo';

  /**
   * Actualiza los parámetros de un efecto Tremolo
   * @param effect Nodo Tremolo a actualizar
   * @param params Parámetros a aplicar
   */
  updateParams(effect: Tone.Tremolo, params: EffectParams): void {
    ...
    console.log('Tremolo actualizado:', {
      frequency: effect.frequency.value,
      depth: effect.depth.value,
  ...
  canUpdate(effect: Tone.ToneAudioNode): boolean {
    return effect instanceof Tone.Tremolo;
  }
   ...
   */
  getCurrentParams(effect: Tone.Tremolo): Record<string, unknown> {
    return {
      frequency: effect.frequency.value,
      depth: effect.depth.value,
   ...
   */
  private safeUpdateParam(effect: Tone.Tremolo, paramName: string, value: number | string): boolean {
    try {
      switch (paramName) {
        case 'frequency':
          ...
          break;
        case 'depth':
          effect.depth.value = value as number;
          break;
        case 'wet':
          if (effect.wet) {
      ...
      console.warn(`⚠️ TremoloUpdater: Error actualizando parámetro ${paramName}:`, error);
      return false;
    }
  }
}
