...
export function ControlPanel() {
  ...
  const { addObject, addEffectZone, addMobileObject, activeGridId, grids, createGrid, currentGridCoordinates, gridSize, showGrid, setShowGrid, setShowParticlePanel, addParticleSystem, updateParticleSystem, selectedEntityId } = useWorldStore();

  const [isParticlesExpanded, setIsParticlesExpanded] = useState(false);
  ...
  }, [selectedShape, selectedColor, selectedSystem, updateParticleSystem]);
  ...
  return (
    <div className="fixed left-0 top-0 h-full z-50 flex">
      ...
      <div className={`relative bg-black border border-white transition-all duration-300 overflow-hidden ${isPanelExpanded ? 'w-80' : 'w-0'
        }`}>
                ...
                <div className="relative border border-white p-3">
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-xs font-mono font-bold text-white tracking-wider flex items-center gap-2">
                      ...
                      <span className="text-cyan-400">007 PARTÍCULAS</span>
                    </h3>
                    <button
                      onClick={() => setIsParticlesExpanded(!isParticlesExpanded)}
                      className="relative border border-white px-2 py-1 text-white hover:bg-white hover:text-black transition-all duration-300 group"
                      title={isParticlesExpanded ? t('controls.hide') : t('controls.show')}
                    >
                      <div className="absolute -inset-0.5 border border-gray-600 group-hover:border-white transition-colors duration-300"></div>
                      <span className="relative text-xs font-mono tracking-wider">
                        {isParticlesExpanded ? t('controls.hide') : t('controls.show')}
                      </span>
                    </button>
                  </div>
                  ...
                  {isParticlesExpanded && (
                    <div className="space-y-4 pt-2">
                      {/* Shapes */}
                      <div>
                        <h3 className="text-[10px] text-cyan-400/70 mb-2 tracking-widest font-bold">SELECT SHAPE</h3>
                        <div className="grid grid-cols-4 gap-1">
                          {shapes.map(s => (
                            <button
                              key={s.id}
                              onClick={() => setSelectedShape(s.id)}
                              className={`p-1 rounded border flex flex-col items-center gap-1 transition-all duration-300 group ${selectedShape === s.id
                                ? 'bg-cyan-500/20 border-cyan-400 text-cyan-200 shadow-[0_0_10px_rgba(34,211,238,0.2)]'
                                : 'border-white/20 bg-black hover:bg-white/10 hover:border-white'
                                }`}
                            >
                      ...
                      {/* Colors */}
                      <div>
                        <h3 className="text-[10px] text-cyan-400/70 mb-2 tracking-widest font-bold">SELECT COLOR</h3>
                        <div className="flex justify-between p-1 bg-white/5 rounded border border-white/10">
                          {colors.map(c => (
                            <button
                              key={c}
                              onClick={() => setSelectedColor(c)}
                              className={`w-6 h-6 rounded-full border transition-all relative ${selectedColor === c
                      ...
                      <button
                        ...
                        className="w-full py-2 bg-gradient-to-r from-cyan-900/50 to-blue-900/50 hover:from-cyan-800 hover:to-blue-800 border border-cyan-500/30 rounded font-bold tracking-[0.1em] text-xs text-cyan-100 transition-all flex items-center justify-center gap-2 relative overflow-hidden group"
                      >
                        <span className="relative z-10">{selectedSystem ? 'UPDATE SYSTEM' : 'ADD SYSTEM'}</span>
                      </button>

                      <div className="text-center">
                        <p className="text-[9px] text-gray-500 tracking-wider">
                          {selectedSystem ? `ID: ${selectedSystem.id.slice(0, 8)}` : 'READY'}
                        </p>
                      </div>
                    </div>
                  )}
