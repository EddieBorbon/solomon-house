...
/**
 * Updater para efectos Vibrato
 * Implementa el Strategy Pattern específico para Vibrato
 */
export class VibratoUpdater implements IEffectUpdater<Tone.Vibrato> {
  readonly effectType = 'Vibrato';

  /**
   * Actualiza los parámetros de un efecto Vibrato
   * @param effect Nodo Vibrato a actualizar
   * @param params Parámetros a aplicar
   */
  updateParams(effect: Tone.Vibrato, params: EffectParams): void {
    ...
    console.log('Vibrato actualizado:', {
  ...
  canUpdate(effect: Tone.ToneAudioNode): boolean {
    return effect instanceof Tone.Vibrato;
  }
   ...
   */
  getCurrentParams(effect: Tone.Vibrato): Record<string, unknown> {
    return {
      frequency: effect.frequency.value,
      depth: effect.depth.value,
   ...
   */
  private safeUpdateParam(effect: Tone.Vibrato, paramName: string, value: number | string): boolean {
    try {
      switch (paramName) {
        case 'frequency':
      ...
      console.warn(`⚠️ VibratoUpdater: Error actualizando parámetro ${paramName}:`, error);
      return false;
    }
  }
}
