...
const targetFile = path.join(messagesDir, 'fr.json');
...
function needsTranslation(value) {
  ...
  const frenchChar = /[a-zA-ZÀ-ÖØ-öø-ÿœŒçÇ]/;
  return !frenchChar.test(trimmed) || /^[A-Z0-9_]+$/.test(trimmed);
}
...
async function translateValue(translate, text) {
  ...
  while (true) {
    attempt += 1;
    try {
      const translated = await translate(prepared, { to: 'fr' });
...
async function main() {
  ...
  for (const entry of entries) {
    ...
    if (currentValue && !needsTranslation(currentValue)) {
      continue;
    }
...
main().catch((error) => {
  console.error('Falló la traducción automática al francés:', error);
  process.exitCode = 1;
});
